...
public class GetArticleBySlugHandler : IRequestHandler<GetArticleBySlugQuery, Result<ArticleDetailDto>>
{
    private readonly IRepository<Article, Guid> _repository;
    private readonly IMemoryCache _cache;

    public GetArticleBySlugHandler(IRepository<Article, Guid> repository, IMemoryCache cache)
    {
        _repository = repository;
        _cache = cache;
    }

    public async Task<Result<ArticleDetailDto>> Handle(GetArticleBySlugQuery request, CancellationToken cancellationToken)
    {
        var cacheKey = $"article_slug_{request.Slug}";
        ...
        var article = await _repository
            .AsQueryable()
            .Where(a => !a.IsDeleted && a.Slug == request.Slug)
